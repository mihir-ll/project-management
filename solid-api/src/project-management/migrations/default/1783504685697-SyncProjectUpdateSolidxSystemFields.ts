...
export class SyncProjectUpdateSolidxSystemFields1783504685697 implements MigrationInterface {
    name = 'SyncProjectUpdateSolidxSystemFields1783504685697'
    public async up(queryRunner: QueryRunner): Promise<void> {
        const tableName = "public.project_update";
...

        {
            ...
            if (table && !table.indices.some((index) => index.name === "IDX_2515df12b5cebe2f36f36154"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "IDX_2515df12b5cebe2f36f36154", columnNames: ["ss_deleted_at"], }));
        }

        {
            const table = await queryRunner.getTable(tableName);
            if (table && !table.indices.some((index) => index.name === "UQ_429f57197e6b37fd8bb73188"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "UQ_429f57197e6b37fd8bb73188", columnNames: ["ss_id"], isUnique: true, }));
        }
    }
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "UQ_429f57197e6b37fd8bb73188");
            if (index)
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "IDX_2515df12b5cebe2f36f36154");
            if (index)
                await queryRunner.dropIndex(tableName, index);
        }
