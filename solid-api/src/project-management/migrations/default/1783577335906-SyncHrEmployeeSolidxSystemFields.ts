...
export class SyncHrEmployeeSolidxSystemFields1783577335906 implements MigrationInterface {
    name = 'SyncHrEmployeeSolidxSystemFields1783577335906'
    public async up(queryRunner: QueryRunner): Promise<void> {
        const tableName = "public.hr_employee";
...

        {
            ...
            if (table && !table.indices.some((index) => index.name === "IDX_69d26cee4667adeb24ac5c35"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "IDX_69d26cee4667adeb24ac5c35", columnNames: ["ss_deleted_at"], }));
        }

        {
            const table = await queryRunner.getTable(tableName);
            if (table && !table.indices.some((index) => index.name === "UQ_6d44f990fff4d26a0217559a"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "UQ_6d44f990fff4d26a0217559a", columnNames: ["ss_id"], isUnique: true, }));
        }
    }
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "UQ_6d44f990fff4d26a0217559a");
            if (index)
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "IDX_69d26cee4667adeb24ac5c35");
            if (index)
                await queryRunner.dropIndex(tableName, index);
        }
