...
export class SyncProjectShareWizardSolidxSystemFields1783510563410 implements MigrationInterface {
    name = 'SyncProjectShareWizardSolidxSystemFields1783510563410'
    public async up(queryRunner: QueryRunner): Promise<void> {
        const tableName = "public.project_share_wizard";
...

        {
            ...
            if (table && !table.indices.some((index) => index.name === "IDX_b7338a4a7b493530b95ebd1f"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "IDX_b7338a4a7b493530b95ebd1f", columnNames: ["ss_deleted_at"], }));
        }

        {
            const table = await queryRunner.getTable(tableName);
            if (table && !table.indices.some((index) => index.name === "UQ_a3c0fba9ab14f4076b9dd319"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "UQ_a3c0fba9ab14f4076b9dd319", columnNames: ["ss_id"], isUnique: true, }));
        }
    }
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "UQ_a3c0fba9ab14f4076b9dd319");
            if (index)
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "IDX_b7338a4a7b493530b95ebd1f");
            if (index)
                await queryRunner.dropIndex(tableName, index);
        }
