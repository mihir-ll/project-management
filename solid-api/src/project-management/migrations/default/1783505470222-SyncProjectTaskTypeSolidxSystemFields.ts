...
export class SyncProjectTaskTypeSolidxSystemFields1783505470222 implements MigrationInterface {
    name = 'SyncProjectTaskTypeSolidxSystemFields1783505470222'
    public async up(queryRunner: QueryRunner): Promise<void> {
        const tableName = "public.project_task_type";
...

        {
            ...
            if (table && !table.indices.some((index) => index.name === "IDX_e093cf2b9c49d2c70bd1ec53"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "IDX_e093cf2b9c49d2c70bd1ec53", columnNames: ["ss_deleted_at"], }));
        }

        {
            const table = await queryRunner.getTable(tableName);
            if (table && !table.indices.some((index) => index.name === "UQ_5b701325e8432ffb7abce3fc"))
                await queryRunner.createIndex(tableName, new TableIndex({ name: "UQ_5b701325e8432ffb7abce3fc", columnNames: ["ss_id"], isUnique: true, }));
        }
    }
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "UQ_5b701325e8432ffb7abce3fc");
            if (index)
        ...
        {
            const table = await queryRunner.getTable(tableName);
            const index = table?.indices.find((entry) => entry.name === "IDX_e093cf2b9c49d2c70bd1ec53");
            if (index)
                await queryRunner.dropIndex(tableName, index);
        }
